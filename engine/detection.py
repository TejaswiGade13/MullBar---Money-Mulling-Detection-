...
def detect_cycles(G: nx.DiGraph, min_len: int = 3, max_len: int = 5, max_results: int = 200) -> list[dict]:
    ...
    Uses NetworkX simple_cycles with length bound, falls back to DFS.
    Only keeps cycles where edges have been used multiple times (frequency ≥ 2)
    to avoid flagging coincidental single-pass cycles in dense graphs.
    ...
    try:
        for cycle in nx.simple_cycles(G, length_bound=max_len):
            ...
            # Only flag cycles with repeated edge usage (not coincidental)
        ...
        rings.extend(_dfs_cycle_detection(G, min_len, max_len))
...
def detect_smurfing(
    ...
    for node in G.nodes():
        if G.in_degree(node) < threshold:
            continue
        incoming = df_ts[df_ts["receiver_id"] == node].sort_values("timestamp")
        if incoming.empty:
            continue

        best = _best_window(incoming, "sender_id", window, threshold)
        if best is not None:
            w_txns, unique_count = best
            members = list(w_txns["sender_id"].unique()) + [node]
            velocity = len(w_txns) / (window_hours / 24)  # txns per day
            rings.append({
                "members": members,
                "pattern_type": "fan_in",
                "aggregator": node,
    ...
    for node in G.nodes():
        if G.out_degree(node) < threshold:
            continue
        outgoing = df_ts[df_ts["sender_id"] == node].sort_values("timestamp")
        if outgoing.empty:
            continue

        best = _best_window(outgoing, "receiver_id", window, threshold)
        if best is not None:
            w_txns, unique_count = best
...
def _best_window(txns: pd.DataFrame, count_col: str, window, threshold: int):
    """Find the densest sliding window meeting the threshold."""
    timestamps = txns["timestamp"].values
    best_result = None
    best_count = 0
    ...
    for i in range(len(txns)):
        ...
        mask = (txns["timestamp"] >= start) & (txns["timestamp"] <= end)
        w = txns[mask]
        uc = w[count_col].nunique()
        if uc >= threshold and uc > best_count:
            best_count = uc
            best_result = (w, uc)
...
def detect_shell_networks(
    G: nx.DiGraph, min_hops: int = 3, max_txn_count: int = 3
) -> list[dict]:
    """
...
def _find_shell_chains(G, node, path, shell_candidates, min_hops, rings, seen_chains, max_depth=7):
    ...
    for successor in G.successors(node):
        ...
        if successor in shell_candidates and len(new_path) < max_depth:
            _find_shell_chains(
                G, successor, path, shell_candidates, min_hops, rings, seen_chains, max_depth
            )
